...
PROJECT_NAME="CincyMuseum-$(date +%Y%m%d%H%M%S)"
...
if aws iam get-role --role-name "$ROLE_NAME" >/dev/null 2>&1; then
  ...
  # Create custom policy with full access permissions for CDK deployment
  # This policy covers all AWS services used in the Museum Chatbot stack:
  # - CloudFormation: CDK stack deployment
  # - IAM: Roles, policies for Lambda, Bedrock, API Gateway
  # - Lambda: Chat, streaming, user CRUD, admin API functions
  # - DynamoDB: User and Conversation History tables
  # - S3: Data buckets, deployments, multimodal content
  # - Bedrock: Knowledge Base, Data Sources, Models, Embeddings
  # - OpenSearch Serverless (AOSS): Vector collection for KB
  # - Secrets Manager: GitHub token storage
  # - Amplify: Frontend hosting and builds
  # - CodeBuild: Amplify build process
  # - CloudWatch Logs: API Gateway and Lambda logging
  # - API Gateway: REST API with streaming
  # - Cognito: Admin user pool authentication
  # - Custom Resources: Amplify build triggers
